...
int main(int argc, char *argv[]) {
    ...
    std::string onnxfilepath = std::string(argv[1]);
    ...
    // Backend
    std::string backend = std::string(argv[2]);
    ...
    if (backend == "cuda") {
        ...
        if (std::find(providers.begin(), providers.end(), "CUDAExecutionProvider") == providers.end()) {
            std::cerr << "onnxruntime is not compiled with the cuda provide, impossible to use onnxruntimecuda backend"
                    << std::endl;
        ...
        OrtStatus *statusPtr = Ort::GetApi().SessionOptionsAppendExecutionProvider_CUDA_V2(
            sessionOptionsCPtr, cuda_options);

        Ort::Status statusCxx(statusPtr);

        if (!statusCxx.IsOK()) {
            std::cerr << "SessionOptionsAppendExecutionProvider_CUDA_V2 returned error " << statusCxx.GetErrorMessage()
                    << std::endl;
            return 1;
        }
    }
    ...
    if (verbose) {
        ...
        for (size_t inputIdx = 0; inputIdx < g_ortSession->GetInputCount(); inputIdx++) {
            std::cout << "  Input " << inputIdx << " name: " << inputNames[inputIdx]
                    << " type: " << convertEnumToString(
                        inputsInfo[inputIdx].GetTensorTypeAndShapeInfo().GetElementType())
        ...
        for (size_t outputIdx = 0; outputIdx < g_ortSession->GetOutputCount(); outputIdx++) {
            std::cout << "  Output " << outputIdx << " name: " << outputNames[outputIdx]
                    << " type: " << convertEnumToString(
                        outputsInfo[outputIdx].GetTensorTypeAndShapeInfo().GetElementType())
